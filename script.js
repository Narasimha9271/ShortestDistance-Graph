//Here we are using vis.js library to create the graph ans 'onload' creates a network visualization of graph
onload = function () {
    ...
    // Function to create graph data(nodes,edges,weights)
    function createData() {
    ...
    //genNew is to randomly generate a new graph
    genNew.onclick = function () {
        // Create new data and display the data
        createData();
        network.setData(curr_data);
        temptext2.innerText =
            "Find least time path from " +
            cities[src - 1] +
            " to " +
            cities[dst - 1];
    ...
    // solve is to shortest path between two nodes using djikstra algorithm
    ...
    function createGraph(data) {
        ...
        for (let i = 0; i < data["edges"].length; i++) {
            ...
            graph[edge["to"] - 1].push([
                edge["from"] - 1,
                parseInt(edge["label"]),
            ]);
            graph[edge["from"] - 1].push([
                edge["to"] - 1,
                parseInt(edge["label"]),
            ]);
        }
    ...
    // function shouldTakePlane checks if it is faster to take a plane or a bus between two nodes
    //The function checks if there is a direct flight between the two nodes, and if so, it returns the weight of the flight.
    // If there is no direct flight, the function returns the weight of the bus ride.
    ...
    // function pushEdges pushes the edges of a path onto a list and edges are pushed in reverse order
    //so the path is displayed from dest to source
